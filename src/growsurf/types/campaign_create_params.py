...
class CampaignCreateParams(TypedDict, total=False):
    ...
    double-sided reward, `HEALTHCARE_PROVIDERS` gets a single-sided reward,
    `SUBSCRIBERS` gets a four-step milestone ladder, and `WAITLIST` gets a leaderboard.
    Every one arrives switched off with a placeholder name, so the program awards
    nothing until you set the amount and turn one on. `TELEHEALTH` is for consumer
    telehealth and wellness subscriptions; `HEALTHCARE_PROVIDERS` is for provider
    networks and clinician-facing products. `INSURANCE` replaces `ONLINE_INSURANCE`,
    which is still accepted. Omit it and every share button keeps its standard default.
    Set only when the program is created; it is not accepted on update.
    """
